...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-30 bg-base-100/95 px-4 shadow-[0_10px_40px_rgba(15,23,42,0.03)] backdrop-blur supports-[backdrop-filter]:bg-base-100/80">
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-4 py-2">
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col leading-tight">
            <span className="text-sm font-semibold tracking-tight">Cursor Notebook</span>
            <span className="text-[11px] uppercase tracking-[0.18em] text-base-content/40">
              Usage &amp; thinking stats
            </span>
          </div>
        </div>
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  className={isActive
                    ? "border-b-2 border-primary pb-1 text-base-content"
                    : "pb-1 text-base-content/70 transition hover:text-base-content"}
                >
                  {item.label}
                </Link>
              );
